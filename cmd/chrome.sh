#!/bin/bash
# Chrome history subcommand (Chromium-based)
VERSION=v0.2.0
SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
source "${SCRIPT_DIR}/../lib/common.sh"

BROWSER_NAME="Chrome"
ENV_VAR="CHROME_HISTORY_DB"

function _get_db_path {
  if [ -n "${CHROME_HISTORY_DB}" ]; then
    echo "${CHROME_HISTORY_DB}"
    return
  fi
  local os=$(uname)
  local base_dir
...
# display help for chrome subcommands.
function help {
...
case "$cmd" in
  ...
  annotations) annotations "$@"; exit $? ;;
  contexts) contexts "$@"; exit $? ;;
